...
class Part1 : Puzzle<Int?> {
    ...
    private fun getScore(map: List<List<Int>>, startPos: Pair<Int, Int>): Int {
        return getScoreRecursive(map, startPos, -1).size;
    }

    private fun getScoreRecursive(map: List<List<Int>>, pos: Pair<Int, Int>, lastNum: Int): Set<Pair<Int, Int>> {
        if (isOutOfBounds(map, pos)) return mutableSetOf();

        val value = map[pos.second][pos.first];
        if (value != lastNum + 1) return mutableSetOf();
        if (value == 9) return mutableSetOf(pos);
